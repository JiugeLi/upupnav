...
# 导出分组数据
npx wrangler d1 execute jiugenav --remote --command "SELECT id, name, icon, sort_order, user_id FROM groups ORDER BY id" --json > /tmp/groups.json

# 导出网站数据
npx wrangler d1 execute jiugenav --remote --command "SELECT id, name, url, description, logo_url, group_id, sort_order, click_count, user_id FROM websites ORDER BY id" --json > /tmp/websites.json
...
for (const group of groups) {
  ...
  try {
    execSync(\`npx wrangler d1 execute jiugenav-dev --remote --command \\\"\${sql}\\\"\`, { stdio: 'pipe' });
